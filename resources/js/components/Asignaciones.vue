        ...
        <div class="col-lg-12">
            <div class="card shadow-lg border-0 rounded-lg mt-12">
              <div class="card-header row justify-content-center">
                <div class="col-md-4"><h4 class="text-center font-weight-light my-4"> Activos: </h4></div>
              </div>
              <div class="card-body">
                <div class="row">
                        ...
                        <table class="table table-bordered table-striped table-sm">
                          ...
                          <tbody v-if="actuales.length">
                              <tr v-for="(detalle,index) in actuales" :key="detalle.id">
                                  <td><button  @click="eliminarDetalle(index)" type="button" class="btn btn-danger btn-sm"><i class="bi bi-x-lg"></i></button></td>
              ...
              <div class="card-footer">
                  <button class="btn btn-primary form-control" type="button" @click="registrarAsignacion()"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-floppy" viewBox="0 0 16 16">
  <path d="M11 2H9v3h2z"/>
    ...
    registrarAsignacion(){
      for (let i = 0; i < this.actuales.length; i++) {
        axios.put('/actual/actualizarasingacion',{
            ...
            'id' : this.actuales[i].id,
            'codigo' : this.actuales[i].codigo,
            'codresp2':this.cod_resp2,
            'codofic2':this.cod_ofi2,
        }).then((response)=>{
          Swal.fire(response.data.message, "", "success");
          window.open('http://senasirprueba.test/actual/repAsignaciones?codofic=' + this.cod_ofi2 + '&codresp='+ this.cod_resp2 +  '','_blank');
